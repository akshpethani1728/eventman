  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7]">
      ...
      <main className="max-w-3xl mx-auto px-4 py-4">
        <div className="flex gap-2 mb-4 card-floating p-1">
          {(["users", "events"] as AdminTab[]).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 h-11 rounded-[14px] text-sm font-medium capitalize transition-all ${
                tab === t ? "bg-indigo-700 text-white shadow-[0_2px_8px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.02)]" : "text-gray-500 hover:text-gray-800"
              }`}>
              ...
              <Card key={u.id} padding="sm">
                ...
                <div className="flex flex-wrap gap-1.5">
                  <select value={u.status} onChange={e => updateUserStatus(u.user_id, e.target.value as any)}
                    className="h-8 px-2 rounded-[14px] border border-gray-200/80 bg-white text-xs flex-1 min-w-0">
                  ...
                  {u.role === "organizer" && (
                    <button onClick={() => toggleOrganizerTrust(u.user_id, u.is_trusted_organizer)}
                      className={`h-8 px-3 rounded-[14px] text-xs font-medium shrink-0 inline-flex items-center gap-1 transition-all ${
                  ...
                  <button onClick={() => deleteUser(u.user_id)}
                    className="h-8 w-8 rounded-[14px] bg-red-50 text-red-600 flex items-center justify-center shrink-0 hover:bg-red-100 transition-colors"
              ...
              <Card key={e.id} padding="sm">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button onClick={() => deleteEvent(e.id)}
                    className="h-8 px-3 rounded-[14px] bg-red-50 text-red-600 text-xs font-medium shrink-0 hover:bg-red-100 transition-colors">
