    ...
    <div className="min-h-screen bg-gradient-to-b from-indigo-50/30 to-gray-50 pb-24">
      ...
      <main className="max-w-lg mx-auto px-4 py-4 space-y-4">
        <form onSubmit={(e) => { e.preventDefault(); saveProfile(); }}>
          <div className="bg-white rounded-2xl p-6 text-center shadow-sm border border-gray-200/70">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-100 to-indigo-200 flex items-center justify-center mx-auto mb-3 shadow-sm ring-2 ring-indigo-200/60">
            ...
            <div className="flex items-center justify-center gap-2 flex-wrap">
              ...
              {profile?.is_trusted_organizer && (
                <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200/60 px-2 py-0.5 rounded-lg">
                  <BadgeCheck className="w-3 h-3" />
                  Trusted
                </span>
              )}
              {!profile?.is_trusted_organizer && profile?.status === "trusted" && (
                <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-indigo-700 bg-indigo-50 border border-indigo-200/60 px-2 py-0.5 rounded-lg">
                  <ShieldCheck className="w-3 h-3" />
                  Verified
                </span>
              )}
              {!profile?.is_trusted_organizer && profile?.status === "basic_verified" && (
                <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-sky-700 bg-sky-50 border border-sky-200/60 px-2 py-0.5 rounded-lg">
          ...
          <div className="bg-white rounded-2xl p-5 space-y-4 shadow-sm border border-gray-200/70">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input value={form.full_name} onChange={e => update("full_name", e.target.value)}
                  className="w-full h-11 pl-10 pr-3.5 rounded-xl border border-gray-200 bg-white text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 transition-all" />
              </div>
            </div>
            ...
            <div>
              ...
              <input value={form.area} onChange={e => update("area", e.target.value)}
                ...
                className="w-full h-11 px-3.5 rounded-xl border border-gray-200 bg-white text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 transition-all" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 space-y-4 shadow-sm border border-gray-200/70">
            ...
            <div>
              ...
              <textarea value={form.bio} onChange={e => update("bio", e.target.value)}
                ...
                className="w-full h-24 px-3.5 py-2.5 rounded-xl border border-gray-200 bg-white text-sm resize-none outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 transition-all" />
            </div>
          </div>
