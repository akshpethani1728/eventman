        ...
        <form onSubmit={(e) => { e.preventDefault(); saveProfile(); }}>
          <div className="bg-white rounded-xl p-6 text-center shadow-sm shadow-black/[0.02] border border-gray-200/70">
            <div className="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-3 ring-1 ring-slate-200">
              <span className="text-2xl font-bold text-slate-700">{profile?.full_name?.charAt(0)?.toUpperCase() || "O"}</span>
            </div>
            <div className="flex items-center justify-center gap-2">
              ...
              {!profile?.is_trusted_organizer && profile?.status === "trusted" && (
                <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-slate-700 bg-slate-100 border border-slate-200/60 px-1.5 py-0.5 rounded-lg">
          ...
          <div className="bg-white rounded-xl p-5 space-y-4 shadow-sm shadow-black/[0.02] border border-gray-200/70">
            ...
            <div>
              ...
              <textarea value={form.bio} onChange={e => update("bio", e.target.value)}
                ...
                className="w-full h-20 px-3 py-2 rounded-lg border border-gray-300 bg-white text-sm resize-none outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500/20 transition-all" />
            </div>
          </div>
