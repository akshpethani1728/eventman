    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-10 h-10 border-2 border-slate-800 border-t-transparent rounded-full animate-spin mx-auto mb-3" />
      ...
      <main className="max-w-3xl mx-auto px-4 py-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-4">
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200/70 shadow-black/[0.02]">
            ...
            <p className="text-[11px] text-gray-500 mt-1 flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-slate-600" /> Active events</p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200/70 shadow-black/[0.02]">
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200/70 shadow-black/[0.02]">
            <p className="text-2xl font-bold text-slate-800">{totalPendingApprovals}</p>
            <p className="text-[11px] text-gray-500 mt-1 flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-slate-600" /> Pending approvals</p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200/70 shadow-black/[0.02]">
        ...
        <div className="flex gap-2 mb-4">
          <button onClick={() => { setCreateFromTemplate(null); setShowCreate(true); }}
            className="flex-1 h-11 rounded-lg bg-slate-800 text-white text-sm font-semibold flex items-center justify-center gap-2 shadow-sm active:scale-[0.97] transition-all duration-150 hover:bg-slate-900">
            <Plus className="w-4 h-4" /> Create Event
          </button>
          {templates.length > 0 && (
            <button onClick={() => setTab("templates")}
              className={`h-11 px-4 rounded-xl border text-sm font-medium flex items-center gap-2 transition-all ${
                tab === "templates" ? "bg-slate-800 text-white border-slate-800 shadow-sm" : "bg-white text-gray-700 border-gray-200/80 shadow-sm active:scale-[0.98]"
              }`}>
        ...
        {tab === "templates" && (
          <div className="space-y-2">
              ...
              <div key={tmpl.id} className="bg-white border border-gray-200 rounded-xl p-3.5 flex items-center justify-between gap-3 shadow-sm">
                ...
                <div className="flex gap-1.5 shrink-0">
                  <button onClick={() => { setCreateFromTemplate(tmpl); setShowCreate(true); }}
                    className="h-8 px-3 rounded-lg bg-slate-800 text-white text-xs font-medium active:scale-95 transition-all">Use</button>
            ...
            <button onClick={() => setTab("active")} className="text-xs text-slate-600 mt-1">&larr; Back to active</button>
          </div>
        )}
          ...
          <div className="space-y-3">
            {sortedActive.length === 0 && (
              <div className="text-center py-16 text-gray-400">
                ...
                <button onClick={() => { setCreateFromTemplate(null); setShowCreate(true); }} className="mt-3 text-sm text-slate-800 font-medium">Create your first event</button>
              </div>
            )}
              ...
              return (
                <div key={event.id}
                  ...
                  <div className="px-4 pt-3.5 pb-2 flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <Link href={`/organizer/events/${event.id}`} className="hover:text-slate-800 transition-colors">
                  ...
                  <div className="px-4 py-2">
                    ...
                    <div className="mb-3">
                      ...
                      <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-500 ${
                          fillPercent >= 100 ? "bg-purple-500" : fillPercent >= 80 ? "bg-emerald-500" : fillPercent >= 50 ? "bg-slate-600" : "bg-amber-500"
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-2.5">
                      {needsApproval && (
                        <button onClick={() => setSelectedEvent(event)}
                          className="text-[10px] font-medium bg-slate-100 text-slate-700 border border-slate-200 px-2 py-0.5 rounded-lg flex items-center gap-1 hover:bg-slate-200 transition-colors">
                    ...
                    {(event.gender_requirement || event.min_age || event.max_age) && (
                      <div className="flex flex-wrap gap-1.5 mb-2.5">
                        ...
                        {event.travel_included && <span className="text-[10px] bg-slate-100 text-slate-700 px-2 py-0.5 rounded-lg">Travel</span>}
                      ...
                      <div className="flex items-center gap-2 py-1.5">
                        <div className="flex -space-x-1.5">
                          {event.recentProfiles.map((p, i) => (
                            <div key={i} className="w-6 h-6 rounded-full bg-slate-200 ring-2 ring-white flex items-center justify-center text-slate-700 font-bold text-[9px]">
                              {p.avatar_url ? (
                        ...
                        <button onClick={() => { setSelectedEvent(event); }}
                          className="text-[10px] text-slate-600 font-medium hover:underline ml-auto">
                          Review all â†’
                        </button>
                      </div>
                    )}
                  ...
                  {isFilling && (
                    <div className="px-4 py-2.5 border-t border-gray-100 flex gap-1.5 overflow-x-auto">
                      ...
                      <Link href={`/organizer/events/${event.id}`}
                        className="h-8 px-3 rounded-lg bg-slate-800 text-white text-xs font-medium flex items-center gap-1.5 hover:bg-slate-900 active:scale-95 transition-all shrink-0">
