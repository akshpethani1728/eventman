        ...
        <form onSubmit={e => e.preventDefault()} className="flex gap-2 mb-3">
          <div className="relative flex-1">
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search by name, area, or skills..."
              className="w-full h-10 pl-9 pr-3 rounded-lg border border-gray-300 bg-white text-sm form-input outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all" />
          </div>
          <button type="button" onClick={() => setShowFilters(!showFilters)}
            className={`h-10 px-3 rounded-lg border text-sm flex items-center gap-1.5 transition-all ${
              showFilters ? "bg-indigo-50 border-indigo-300 text-indigo-700" : "border-gray-300 text-gray-600 bg-white"
            }`}>
          ...
          <form onSubmit={e => e.preventDefault()} className="bg-white border border-gray-200 rounded-xl p-3 mb-3 space-y-2">
            ...
            {(filters.gender || filters.availability || filters.city || filters.skills) && (
              <button type="button" onClick={() => setFilters({ gender: "", availability: "", city: "", skills: "" })}
                className="text-xs text-indigo-600 flex items-center gap-1"><X className="w-3 h-3" /> Clear filters</button>
            )}
          </form>
        )}
              ...
              <Card key={w.id} padding="sm" hover className="cursor-pointer transition-all duration-200 active:scale-[0.99] group"
                ...
                <div className="flex items-start gap-3">
                  <div className="relative shrink-0">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-50 to-indigo-100 flex items-center justify-center text-indigo-700 font-semibold text-sm group-hover:ring-2 group-hover:ring-indigo-300 transition-all">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-medium text-sm text-gray-900 group-hover:text-indigo-700 transition-colors">{w.full_name}</p>
                      {avail && (
