...
export default function WorkerProfilePanel({ worker, organizerId, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white shadow-sm shadow-black/[0.03] animate-slide-right overflow-y-auto overscroll-behavior-contain">
        {/* Close button */}
        <button onClick={onClose} className="absolute top-4 right-4 z-10 w-8 h-8 rounded-2xl bg-white/90 backdrop-blur-sm shadow-sm flex items-center justify-center text-gray-500 hover:text-gray-800 hover:bg-white transition-all">
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="bg-gradient-to-br from-indigo-700 via-indigo-800 to-indigo-950 px-5 pt-10 pb-6">
          <div className="flex items-end gap-4">
            <div className="relative shrink-0">
              <div className="w-16 h-16 rounded-3xl bg-white/20 backdrop-blur-sm flex items-center justify-center text-white text-2xl font-bold ring-2 ring-white/30 shadow-sm shadow-black/[0.03]">
                {worker.avatar_url ? (
                  <img src={worker.avatar_url} alt="" className="w-16 h-16 rounded-3xl object-cover" />
                ) : (
        ...
        <div className="p-5 space-y-5">
            ...
            {completion.missing.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {completion.missing.map(m => (
                  <span key={m} className="text-[10px] bg-amber-50 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-xl flex items-center gap-1">
                ...
                <div>
                  <div className="flex flex-wrap gap-1.5">
                    {worker.skills.map((s, i) => (
                      <span key={i} className="text-[11px] bg-indigo-50 text-indigo-700 border border-indigo-200 px-2.5 py-1 rounded-xl font-medium">{s}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          {/* Contact (conditional) */}
          {canViewContact && (
            <div className="bg-white rounded-2xl border border-emerald-200/80 overflow-hidden">
